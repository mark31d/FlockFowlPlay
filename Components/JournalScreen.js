...
export default function JournalScreen() {
  ...
  // три режима: список, детали, форма
  ...
  // переключение в режим добавления
  function startAdd() {
    setFormMode('add');
    setEditingId(null);
    setDate(new Date());
      ...
      <SafeAreaView style={st.wrap}>
        ...
        <ScrollView contentContainerStyle={st.form}>
          <DateTimePicker
            ...
            textColor={TXT}
          />
          <Text style={st.dateText}>{fmt(date)}</Text>
    ...
    return (
      <SafeAreaView style={st.wrap}>
        ...
        <ScrollView>
          ...
          <View style={st.pad}>
            {e.tag && <View style={st.badge_first}><Text style={st.badgeTxt_first}>{e.tag}</Text></View>}
      ...
      {filtered.length === 0 ? (
        <View style={st.empty}>
          ...
          <View style={st.bubbleWrap}>
            <View style={st.bubble}><Text style={st.bubbleTxt}>
...
renderItem={({item})=>(
  <TouchableOpacity onPress={()=>setSelected(item)}>
    <View style={st.card}>
      ...
      {/* 2. chips on the right, wrapping if too many */}
        ...
        <View style={st.sheet}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={st.chipWrap}>
            {SEASONS.map(s=>{
              ...
              return (
                <TouchableOpacity key={s} style={[st.chip, sel && st.chipActive]} onPress={()=>setFS(sel?null:s)}>
